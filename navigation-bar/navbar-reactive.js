        ...
        if (!excludeIds.includes(button.id)) {
            ...
            if (button.id !== "link-log-out") {
                ...
                btn.addEventListener("click", function (_) {
                    ...
                    window.location.href = "/" + window.location.pathname.split('/').at(1) + "/index/index.html";
                });
            }
...
function loadNavbar() {
    fetch("/navigation-bar/navbar.json") // Reemplaza con la ruta real del JSON
        ...
        .then(async data => {
            ...
            if (currentUser === null) {
                await loadNavbarForGuestUser(data);
            } else {
                await loadNavbarForAuthenticatedUser(data);
            }
        })
